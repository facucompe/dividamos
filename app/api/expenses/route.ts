...
async function getFileFromGitHub(): Promise<{ content: ExpenseData; sha: string }> {
...
async function updateFileInGitHub(content: ExpenseData, sha: string): Promise<void> {
  if (!GITHUB_TOKEN || !GITHUB_REPO) {
    // Fallback to local file if GitHub not configured
    const fs = await import('fs/promises');
    const path = await import('path');
...
export async function POST(request: Request) {
  try {
    const newData: ExpenseData = await request.json();
